"use client";

import { use, useState } from "react";

import { DisbursementForm } from "@/components/loans/disbursement-form";
import { InstallmentStatusBadge } from "@/components/loans/installment-status-badge";
import { LoanStatusBadge } from "@/components/loans/loan-status-badge";
import { PayoutDetailsReveal } from "@/components/loans/payout-details-reveal";
import { ManualReminderButton } from "@/components/messaging/manual-reminder-button";
import { PaymentHistoryTable } from "@/components/repayments/payment-history-table";
import { RepaymentForm } from "@/components/repayments/repayment-form";
import { Button } from "@/components/ui/button";
import { Card, CardContent } from "@/components/ui/card";
import { useAgreement } from "@/features/agreements/use-agreements";
import { useAdminLoan, useApproveLoan } from "@/features/loans/use-loans";
import { formatDate, formatDateTime, formatGHS } from "@/lib/format";

export default function AdminLoanDetailPage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = use(params);
  const { data: loan, isLoading, isError } = useAdminLoan(id);
  const { data: agreement } = useAgreement(loan?.agreement_id ?? "");
  const approveLoan = useApproveLoan(id);
  const [approveError, setApproveError] = useState<string | null>(null);

  const handleApprove = async () => {
    setApproveError(null);
    try {
      await approveLoan.mutateAsync();
    } catch {
      setApproveError("Couldn't approve this loan. It may already have been actioned.");
    }
  };

  if (isLoading) return <p className="text-muted-foreground p-6 text-sm">Loading…</p>;
  if (isError || !loan) {
    return <p className="text-destructive p-6 text-sm">Couldn&apos;t load this loan.</p>;
  }

  return (
    <main className="flex flex-1 flex-col gap-6 p-6">
      <div className="flex items-center justify-between gap-4">
        <div>
          <h1 className="text-foreground text-2xl font-semibold tracking-tight">
            {loan.loan_number}
          </h1>
          <p className="text-muted-foreground text-sm">
            {loan.customer_name || loan.customer_email} — {loan.customer_phone} — Request{" "}
            {loan.request_number}
          </p>
        </div>
        <LoanStatusBadge status={loan.status} />
      </div>

      <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
        <Card size="sm">
          <CardContent>
            <p className="text-muted-foreground text-xs">Principal</p>
            <p className="text-foreground text-lg font-semibold">{formatGHS(loan.principal)}</p>
          </CardContent>
        </Card>
        <Card size="sm">
          <CardContent>
            <p className="text-muted-foreground text-xs">Total repayable</p>
            <p className="text-foreground text-lg font-semibold">
              {formatGHS(loan.total_repayable)}
            </p>
          </CardContent>
        </Card>
        <Card size="sm">
          <CardContent>
            <p className="text-muted-foreground text-xs">Disbursed</p>
            <p className="text-foreground text-lg font-semibold">
              {formatGHS(loan.amount_disbursed)}
            </p>
          </CardContent>
        </Card>
        <Card size="sm">
          <CardContent>
            <p className="text-muted-foreground text-xs">Outstanding</p>
            <p className="text-foreground text-lg font-semibold">
              {formatGHS(loan.outstanding_balance)}
            </p>
          </CardContent>
        </Card>
      </div>

      {agreement && (
        <Card size="sm" className="max-w-md">
          <CardContent className="flex flex-col gap-2">
            <p className="text-muted-foreground text-xs">Loan agreement</p>
            <p className="text-foreground text-sm">
              Signed by {agreement.typed_legal_name} on {formatDateTime(agreement.accepted_at)} —
              email {agreement.email_delivery_status.toLowerCase()}
            </p>
            <Button
              variant="outline"
              size="sm"
              className="self-start"
              nativeButton={false}
              render={
                <a href={agreement.download_url} target="_blank" rel="noopener noreferrer">
                  Download agreement (PDF)
                </a>
              }
            />
          </CardContent>
        </Card>
      )}

      {loan.status === "PENDING_APPROVAL" && (
        <div className="flex flex-col items-start gap-2">
          <Button onClick={handleApprove} disabled={approveLoan.isPending}>
            {approveLoan.isPending ? "Approving…" : "Approve for disbursement"}
          </Button>
          {approveError && <p className="text-destructive text-sm">{approveError}</p>}
        </div>
      )}

      {loan.status === "APPROVED_FOR_DISBURSEMENT" && (
        <div className="flex flex-col gap-3">
          <h2 className="text-foreground text-base font-semibold tracking-tight">Record disbursement</h2>
          <PayoutDetailsReveal loanId={loan.id} />
          <DisbursementForm loan={loan} />
        </div>
      )}

      {loan.disbursement && (
        <Card size="sm" className="max-w-md">
          <CardContent className="flex flex-col gap-2">
            <p className="text-muted-foreground text-xs">Disbursement record</p>
            <div className="flex justify-between text-sm">
              <span className="text-muted-foreground">Amount</span>
              <span className="text-foreground font-medium">
                {formatGHS(loan.disbursement.amount)}
              </span>
            </div>
            <div className="flex justify-between text-sm">
              <span className="text-muted-foreground">Method</span>
              <span className="text-foreground font-medium">{loan.disbursement.method}</span>
            </div>
            {loan.disbursement.external_transaction_reference && (
              <div className="flex justify-between text-sm">
                <span className="text-muted-foreground">Reference</span>
                <span className="text-foreground font-medium">
                  {loan.disbursement.external_transaction_reference}
                </span>
              </div>
            )}
            <div className="flex justify-between text-sm">
              <span className="text-muted-foreground">Recorded by</span>
              <span className="text-foreground font-medium">
                {loan.disbursement.recorded_by_name}
              </span>
            </div>
            <div className="flex justify-between text-sm">
              <span className="text-muted-foreground">Recorded at</span>
              <span className="text-foreground font-medium">
                {formatDateTime(loan.disbursement.recorded_at)}
              </span>
            </div>
          </CardContent>
        </Card>
      )}

      {loan.installments.length > 0 && (
        <div>
          <h2 className="text-foreground mb-2 text-base font-semibold tracking-tight">Repayment schedule</h2>
          <div className="bg-card ring-border shadow-card overflow-x-auto rounded-[1.125rem] ring-1">
            <table className="w-full text-sm">
              <thead>
                <tr className="border-border text-muted-foreground border-b text-left">
                  <th className="px-5 py-3 font-medium">#</th>
                  <th className="px-5 py-3 font-medium">Due date</th>
                  <th className="px-5 py-3 font-medium">Total due</th>
                  <th className="px-5 py-3 font-medium">Outstanding</th>
                  <th className="px-5 py-3 font-medium">Status</th>
                  <th className="px-5 py-3 font-medium">Action</th>
                </tr>
              </thead>
              <tbody>
                {loan.installments.map((installment) => (
                  <tr
                    key={installment.sequence_number}
                    className="border-border hover:bg-muted/60 border-b transition-colors last:border-0"
                  >
                    <td className="px-5 py-3">{installment.sequence_number}</td>
                    <td className="px-5 py-3">{formatDate(installment.due_date)}</td>
                    <td className="px-5 py-3">{formatGHS(installment.total_due)}</td>
                    <td className="px-5 py-3">{formatGHS(installment.outstanding_amount)}</td>
                    <td className="px-5 py-3">
                      <InstallmentStatusBadge status={installment.status} />
                    </td>
                    <td className="px-5 py-3">
                      {installment.status !== "PAID" && installment.status !== "WAIVED" && (
                        <ManualReminderButton installmentId={installment.id} />
                      )}
                    </td>
                  </tr>
                ))}
              </tbody>
            </table>
          </div>
        </div>
      )}

      {(loan.status === "ACTIVE" || loan.status === "OVERDUE") && (
        <div className="flex flex-col items-start gap-3">
          <h2 className="text-foreground text-base font-semibold tracking-tight">Repayments</h2>
          <RepaymentForm loan={loan} />
        </div>
      )}

      {loan.payments.length > 0 && (
        <div>
          <h2 className="text-foreground mb-2 text-base font-semibold tracking-tight">Payment history</h2>
          <PaymentHistoryTable loanId={loan.id} payments={loan.payments} showActions />
        </div>
      )}
    </main>
  );
}
